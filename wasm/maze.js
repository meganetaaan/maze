...
import _Math$clz from 'babel-runtime/core-js/math/clz32';
...
var Module;if (!Module) Module = (typeof Module !== 'undefined' ? Module : null) || {};var moduleOverrides = {};for (var key in Module) {
...
}Module['stackTrace'] = stackTrace;var WASM_PAGE_SIZE = 65536;var ASMJS_PAGE_SIZE = 16777216;var MIN_TOTAL_MEMORY = 16777216;function alignUp(x, multiple) {
  if (x % multiple > 0) {
  ...
  byteLength = Function.prototype.call.bind(_Object$getOwnPropertyDescriptor(ArrayBuffer.prototype, 'byteLength').get);byteLength(new ArrayBuffer(4));
} catch (e) {
...
};Math.trunc = Math['trunc'];var Math_abs = Math.abs;var Math_cos = Math.cos;var Math_sin = Math.sin;var Math_tan = Math.tan;var Math_acos = Math.acos;var Math_asin = Math.asin;var Math_atan = Math.atan;var Math_atan2 = Math.atan2;var Math_exp = Math.exp;var Math_log = Math.log;var Math_sqrt = Math.sqrt;var Math_ceil = Math.ceil;var Math_floor = Math.floor;var Math_pow = Math.pow;var Math_imul = _Math$imul;var Math_fround = _Math$fround;var Math_round = Math.round;var Math_min = Math.min;var Math_clz32 = _Math$clz;var Math_trunc = _Math$trunc;var runDependencies = 0;var runDependencyWatcher = null;var dependenciesFulfilled = null;function getUniqueRunDependency(id) {
  return id;
  ...
  var method = Module['wasmJSMethod'] || 'native-wasm';Module['wasmJSMethod'] = method;var wasmTextFile = Module['wasmTextFile'] || 'maze.wast';var wasmBinaryFile = Module['wasmBinaryFile'] || 'maze.wasm';var asmjsCodeFile = Module['asmjsCodeFile'] || 'maze.asm.js';if (typeof Module['locateFile'] === 'function') {
    ...
    global = fixImports(global);env = fixImports(env);if (!env["table"]) {
      ...
      env["tableBase"] = 0;
    }var exports;exports = doNativeWasm(global, env, providedBuffer);return exports;
  };var methodHandler = Module['asm'];
}integrateWasmJS(Module);var ASM_CONSTS = [];STATIC_BASE = Runtime.GLOBAL_BASE;STATICTOP = STATIC_BASE + 15328;__ATINIT__.push();memoryInitializer = Module['wasmJSMethod'].indexOf('asmjs') >= 0 || Module['wasmJSMethod'].indexOf('interpret-asm2wasm') >= 0 ? 'maze.js.mem' : null;var STATIC_BUMP = 15328;Module['STATIC_BASE'] = STATIC_BASE;Module['STATIC_BUMP'] = STATIC_BUMP;var tempDoublePtr = STATICTOP;STATICTOP += 16;function __ZSt18uncaught_exceptionv() {
...
}function ___lock() {}function ___unlock() {}function _pthread_mutexattr_init() {}var PTHREAD_SPECIFIC = {};function _pthread_getspecific(key) {
...
}function _pthread_mutexattr_destroy() {}function ___cxa_allocate_exception(size) {
  return _malloc(size);
  ...
  if (Module['___errno_location']) HEAP32[Module['___errno_location']() >> 2] = value;return value;
    ...
    var total = 0;var create = [];_Object$keys(src.entries).forEach(function (key) {
      var e = src.entries[key];var e2 = dst.entries[key];if (!e2 || e.timestamp > e2.timestamp) {
        create.push(key);total++;
      }
    });var remove = [];_Object$keys(dst.entries).forEach(function (key) {
    ...
    if (!FS.FSNode) {
      ...
      };FS.FSNode.prototype = {};var readMode = 292 | 73;var writeMode = 146;_Object$defineProperties(FS.FSNode.prototype, { read: { get: function get() {
    ...
    if (!FS.FSStream) {
      FS.FSStream = function () {};FS.FSStream.prototype = {};_Object$defineProperties(FS.FSStream.prototype, { object: { get: function get() {
            return this.node;
          }, set: function set(val) {
            this.node = val;
    ...
    var lookup = FS.lookupPath(mountpoint, { follow_mount: false });if (!FS.isMountpoint(lookup.node)) {
      throw new FS.ErrnoError(ERRNO_CODES.EINVAL);
    }var node = lookup.node;var mount = node.mounted;var mounts = FS.getMounts(mount);_Object$keys(FS.nameTable).forEach(function (hash) {
      var current = FS.nameTable[hash];while (current) {
    ...
    function LazyUint8Array() {
      ...
      if (!ENVIRONMENT_IS_WORKER) throw "Cannot do synchronous binary XHRs outside webworkers in modern browsers. Use --embed-file or --preload-file in emcc";var lazyArray = new LazyUint8Array();_Object$defineProperties(lazyArray, { length: { get: function get() {
            if (!this.lengthKnown) {
              this.cacheLength();
            }return this._length;
    ...
    }_Object$defineProperties(node, { usedBytes: { get: function get() {
          return this.contents.length;
        } } });var stream_ops = {};var keys = _Object$keys(node.stream_ops);keys.forEach(function (key) {
...
};var _wasm_array_to_js = Module['_wasm_array_to_js'] = function () {
  return Module["asm"]["_wasm_array_to_js"].apply(null, arguments);
  ...
  if (Module['onAbort']) {
    ...
    Module.print(what);Module.printErr(what);what = _JSON$stringify(what);
  } else {
    what = '';
